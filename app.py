...
st.set_page_config(
    page_title="Pacing Triatlón — Pablo Iglesias Navarrete",
    layout="centered"
)
...
df_703 = pd.DataFrame(
    data_703,
    columns=["Experiencia", "Peso", "Desnivel", "IF_min", "IF_max"]
)
...
tiene_grasa = st.radio(
    "¿Tienes el dato de % de grasa?",
    ["No", "Sí"]
)
...
desnivel = st.selectbox(
    "Desnivel del circuito",
    ["Bajo", "Medio", "Alto"]
)
...
if fila.empty:
    ...
    st.write(
        f"**Subidas:** {pot_subidas:.0f} W "
        f"({pot_subidas / peso:.2f} W/kg)"
    )
    st.write(
        f"**Llano:** {pot_llano:.0f} W "
        f"({pot_llano / peso:.2f} W/kg)"
    )
    st.write(
        f"**Bajadas:** {pot_bajadas:.0f} W "
        f"({pot_bajadas / peso:.2f} W/kg)"
    )
